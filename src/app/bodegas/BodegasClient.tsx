  ...
  return (
    <div className="space-y-6 w-full">
      ...
      <div className="space-y-4 w-full">
        {/* Controls: Search + Filter + New Button */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm">
          <div className="relative flex-1 w-full sm:max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 text-xs bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-800 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500 font-medium"
            />
          </div>

          <div className="flex items-center gap-2.5 w-full sm:w-auto">
            <div className="flex items-center gap-2 flex-1 sm:flex-initial">
              <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider shrink-0 hidden sm:inline-block">
                Sucursal:
              </span>
              <select
                value={selectedSucursalFilter}
                onChange={(e) => setSelectedSucursalFilter(e.target.value)}
                className="px-3 py-2 text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-teal-500 font-semibold shrink-0"
              >
            ...
            {isAdmin && (
              <button
                ...
                onClick={() => {
                  setNewNombre("");
                  setNewSucursalId(sucursales[0]?.id || "");
                  setIsCreateModalOpen(true);
                }}
                className="inline-flex items-center gap-1.5 px-4 py-2 bg-[#162158] hover:bg-[#0f173d] dark:bg-teal-600 dark:hover:bg-teal-500 text-white text-xs font-extrabold rounded-xl transition-all shadow-sm active-scale-down cursor-pointer shrink-0"
              >
        ...
        {/* Cards Grid a 3 columnas para visualización amplia y limpia */}
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 bg-[#162158] hover:bg-[#0f1842] dark:bg-teal-600 dark:hover:bg-teal-500 text-white font-extrabold text-xs rounded-xl transition-all shadow-sm active-scale-down cursor-pointer disabled:opacity-50 flex items-center justify-center gap-1"
                >
                  <Plus className="h-4 w-4" />
                  <span>{isCreating ? "Registrando..." : "Guardar"}</span>
                </button>
              </div>
            </form>
          </div>
